...
%save_path_base='/data01/user-storage/y.zezhang/2024_subsample_project/observer_study/pilot_study';
save_path_base='/datastore01/user-storage/y.zezhang/image_inspect/images';
%sa_folder=append('/data01/user-storage/y.zezhang/2024_subsample_project/mod_SA_images/',num2str(subsample_level));
...
%observer_study_path='/data01/user-storage/y.zezhang/2024_subsample_project/document/observer_study_patient_list/mannual selection/pilot';
...
%def_image_select = textscan(fid, '%s %s', 'Delimiter', ' ');
fclose(fid);
...
for i=1:1:length(hl_image_select)
    %mkdir(fullfile(save_path_base,num2str(subsample_level),'grey_scale','hl'));
    %mkdir(fullfile(save_path_base,num2str(subsample_level),'ge','hl'));
    for dose_index=1:1:length(subsample_level)
        subsample_cur=subsample_level{dose_index};

        sa_folder=fullfile( sa_folder_base,subsample_level);
        ...
        mkdir(fullfile(save_path_base,subsample_cur,'grey_scale','hl'));
        mkdir(fullfile(save_path_base,subsample_cur,'ge','hl'));
        ...
        try
            ...
            gif_filename=append('pat',pat_No,'_d',subsample_cur,'.gif');
            gif_file_save_path=fullfile(save_path_base,subsample_cur,'grey_scale','hl',gif_filename);
            ...
            dims=[64,64,str2num(subsample_cur)];
            %gif_filename=append(pat_No,'_','hl','_',subsample_cur,'.gif');
            %gif_file_save_path=fullfile(save_path_base,subsample_cur,gif_filename);
            gif_filename=append('pat',pat_No,'_d',subsample_cur,'.gif');
            gif_file_save_path=fullfile(save_path_base,subsample_cur,'grey_scale','hl',gif_filename);
            delay_time=0.1*(30/str2num(subsample_cur));
        ...
        lowdose=append('reoriented_windowed.img');
        ...
        save_filename_path=fullfile(save_path_base,subsample_cur,'grey_scale','hl');
        png_save_name=append('pat',pat_No,'_d',subsample_cur);
        ...
        ge_save_filename_path_lowdose=fullfile(save_path_base,subsample_cur,'ge','hl',lowdose_png);
...
for i=1:1:length(def_image_select)
    for dose_index=1:1:length(subsample_level)
        ...
        mkdir(fullfile(save_path_base,subsample_cur,'grey_scale',def_type_cur));
        mkdir(fullfile(save_path_base,subsample_cur,'ge',def_type_cur))
        ...
        try
            ...
            dims=[64,64,str2num(subsample_cur)];
            %gif_filename=append(pat_No,subsample_cur,'.gif');
            gif_filename=append('pat',pat_No,'_d',subsample_cur,'_',def_type_cur,'.gif');
            gif_file_save_path=fullfile(save_path_base,subsample_cur,'grey_scale',def_type_cur,gif_filename);
            %gif_file_save_path=fullfile(save_path_base,subsample_cur,gif_filename);
            delay_time=0.1*(30/str2num(subsample_cur));
        ...
        save_filename_path=fullfile(save_path_base,subsample_cur,'grey_scale',def_type_cur);
        png_save_name=append('pat',pat_No,'_d',subsample_cur,'_',def_type_cur);
        generate_image_series_v2_png_only(lowdose,save_filename_path,png_save_name);
        ...
        grey_load_filename_path_lowdose=fullfile(save_filename_path,lowdose_png);
        ge_save_filename_path_lowdose=fullfile(save_path_base,subsample_cur,'ge',def_type_cur,lowdose_png);
