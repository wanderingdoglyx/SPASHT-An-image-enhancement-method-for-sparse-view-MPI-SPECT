...
file_path = "/home/y.zezhang/2024_subsample_project/CHO_observer_study_nn2/patient_selection.txt"
...
for location in location_setting:
  for extent in extent_setting:
    for severity in severity_setting:
          for diag in ['diseased', 'healthy']:
            for index, di_item in enumerate(diseased_patients):
              ...
              if diag == 'diseased':
                ...
                cur_path=os.path.join(base_folder,def_name)
                SA_fname='recon_pat'+ patient +'_'+def_name+'_d'+str(subsample_level)+'_it8_b'+str(bt_size)+'_lmbdchdiff'+str(lambda_val_ind_chdiff)+'_lmbdmdiff'+str(lambda_val_ind_mdiff)+'.img'
          ...
          savemat(save_folder +'SNR_data'+str(dl_version)+'_d'+str(subsample_level)+'_location'+str(location)+"_extent"+str(extent)+'_severity'+str(severity)+'.mat', mdic)
