...
dose_level=1
isIO=0
location_setting=['a']  # ['a','i']
extent_setting = [60]  #[30,45,60] 
...
root_folder = f'{base_folder}/test_data_sa_wd_fix2_48cube'
root_folder_mirirv3 = f'{base_folder}/test_data_mirirv3_sa_wd_48cube'
root_folder_prev = f'{base_folder}/test_data_sa_wd'
root_folder_mirirv3_prev = f'{base_folder}/test_data_mirirv3_sa_wd'
save_folder = f'{base_folder}/SNR/d{dose_level}/'
        ...
        for extent in extent_setting:
          for severity in severity_setting:
            ...
            if src_name == 'mirirv3':
              ...
              root_folder_prev_act = root_folder_mirirv3_prev
            else:
...
np.save(save_folder +'delta_f_bar_'+'_d'+str(dose_level)+'_location'+str(location_setting)+"_extent"+str(extent_setting)+'_severity'+str(severity_setting),delta_f_bar)
np.save(save_folder +"delta_v_bar_"+'_d'+str(dose_level)+'_location'+str(location_setting)+"_extent"+str(extent_setting)+'_severity'+str(severity_setting),delta_v_bar)
np.save(save_folder + "K_"+'_d'+str(dose_level)+'_location'+str(location_setting)+"_extent"+str(extent_setting)+'_severity'+str(severity_setting),K)
...
mdic={'delta_f_bar'+'_d'+str(dose_level): delta_f_bar,
      "delta_v_bar"+'_d'+str(dose_level): delta_v_bar,
      "K"+'_d'+str(dose_level): K,
      "IS_"+'_d'+str(dose_level): np.mean(IS,axis=1),
      "IN_"+'_d'+str(dose_level):np.mean(IN,axis=1)}

savemat(save_folder +'SNR_data'+'_d'+str(dose_level)+'_location'+str(location_setting)+"_extent"+str(extent_setting)+'_severity'+str(severity_setting)+'.mat', mdic)
