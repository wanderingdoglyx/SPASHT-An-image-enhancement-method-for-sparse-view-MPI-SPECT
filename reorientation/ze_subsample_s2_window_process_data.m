function ze_subsample_s2_window_process_data(category, AC_method, ind_pat, patient_list,slice_number)
    ...
    if strcmp(category,'healthy')
        ...
        fname = [seg_dir,'/def_center/',pat_id,'/def_centroid_','dil2115','_mod_again5.bin'];

        f = fopen(fname);
        if f < 0
            fname = [seg_dir,'/def_center/',pat_id,'/def_centroid_','dil2115','_mod_apr3.bin'];
            f = fopen(fname);
        end
        ...
        for ind_loc = 1:length(loc_arr)
            ...
            for def_sev = sev_arr
                ...
                fname = [seg_dir,'/def_center/',pat_id,'/def_centroid_',def_loc,'21',num2str(def_ext),'_mod_again5.bin'];
                f = fopen(fname);
                if f < 0
                fname = [seg_dir,'/def_center/',pat_id,'/def_centroid_',def_loc,'21',num2str(def_ext),'_mod_apr3.bin'];
                f = fopen(fname);
                end
                def_c = fread(f,'float32');
                fclose(f);
