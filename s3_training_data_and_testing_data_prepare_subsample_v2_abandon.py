...
def copy_subfolders(source_folder, subfolder_list, destination, exclude_set):
    """
    ...
    for subfolder in subfolder_list:
        if subfolder not in exclude_set:
            try:
                ...
                print(f"Failed to copy {subfolder}: {e}")
...
def_destination1 = '/path/to/destination1'
hl_destination2 = '/path/to/destination2'

# Create destination directories if they do not exist
os.makedirs(def_destination1, exist_ok=True)
os.makedirs(hl_destination2, exist_ok=True)
...
copied_subfolders_dest1 = set()

copied_subfolders_dest1 = copy_subfolders(source_main_folder, group1_dest1_subfolders, def_destination1, copied_subfolders_dest1)

# Copy all remaining subfolders from group2 to destination2
copied_subfolders_dest2 = set()

remaining_group2_subfolders = [subfolder for subfolder in all_group2_subfolders if subfolder not in copied_subfolders_dest1]
copied_subfolders_dest2 = copy_subfolders(source_main_folder, remaining_group2_subfolders, hl_destination2, copied_subfolders_dest1)

print(f"Subfolders copied to '{def_destination1}': {copied_subfolders_dest1}")
print(f"Subfolders copied to '{hl_destination2}': {copied_subfolders_dest2}")
