...
def categorize_by_gender(file_path):
    # Initialize lists for categorizing the first column
    male_list = []
    female_list = []
    ...
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            ...
            if len(columns) >= 2:
                ...
                # Categorize based on the gender
                if gender == "M":
                    male_list.append(first_column)
                elif gender == "F":
                    female_list.append(first_column)
...
'''
file_path_def = '/data01/user-storage/y.zezhang/2024_subsample_project/document/zitong_patient_list/combined_v2_final_test_pat_list_def.txt'
male_list_def, female_list_def = categorize_by_gender(file_path_def)

file_path_hl = '/data01/user-storage/y.zezhang/2024_subsample_project/document/zitong_patient_list/combined_v2_final_test_pat_list_hl.txt'
male_list_hl, female_list_hl = categorize_by_gender(file_path_hl)
'''
#file_path = "patient_selection.txt"
#diseased_patients, healthy_patients = read_patient_file(file_path)

pat_id_arr_fname = f'/data01/user-storage/y.zezhang/2024_subsample_project/document/zitong_patient_list/ze_female_diseased_patient_ids.txt'
diseased_patients = np.loadtxt(pat_id_arr_fname, dtype = 'str', comments="#", delimiter=",", unpack=False)

pat_id_arr_fname = f'/data01/user-storage/y.zezhang/2024_subsample_project/document/zitong_patient_list/ze_female_healthy_patient_ids.txt'
healthy_patients = np.loadtxt(pat_id_arr_fname, dtype = 'str', comments="#", delimiter=",", unpack=False)

#diseased_patients, healthy_patients = female_list_def,female_list_hl
...
for study_project in observer_study_list:
  ...
  fname_tSN = f'{save_folder}/t_ZT_fe_loc_{location_setting_combined}_sev_{severity_setting_combined}_ext_{extent_setting_combined}_subsample{study_project}_IO{isIO}_Ud{Ud}.txt'
