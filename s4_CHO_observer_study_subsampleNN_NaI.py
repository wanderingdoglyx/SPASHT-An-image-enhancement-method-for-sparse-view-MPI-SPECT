...
pat_id_arr_fname = f'/data01/user-storage/y.zezhang/2024_subsample_project/document/collimator_record/nai_testing_patients_def.txt'
diseased_patients = np.loadtxt(pat_id_arr_fname, dtype = 'str', comments="#", delimiter=",", unpack=False)

pat_id_arr_fname = f'/data01/user-storage/y.zezhang/2024_subsample_project/document/collimator_record/nai_testing_patients_hl.txt'
healthy_patients = np.loadtxt(pat_id_arr_fname, dtype = 'str', comments="#", delimiter=",", unpack=False)
...
for study_project in observer_study_list:
  ...
  fname_tSN = f'{save_folder}/t_ZT_loc_{location_setting_combined}_sev_{severity_setting_combined}_ext_{extent_setting_combined}_subsample{study_project}_IO{isIO}_Ud{Ud}_NaI.txt'
