...
def run_cho(dl_version, Ud, dose_level, isIO, bt_size, lambda_val_ind_chdiff, lambda_val_ind_mdiff, gender):

  base_folder = '/data/rahman.m/projects/dl_denoising/debug/db_defect_insertion/data_spie/'
  root_folder = f'{base_folder}/learning/v1_july_22_unet_3d/den_3d_v{dl_version}/pred';
  root_folder_mirirv3 = f'{base_folder}/learning/v1_july_22_unet_3d/den_3d_v{dl_version}/pred_mirirv3'
  root_folder_prev = f'{base_folder}/test_data_sa_wd';
  root_folder_prev_mirirv3 = f'{base_folder}/test_data_mirirv3_sa_wd';
  save_folder = f'{base_folder}/learning/v1_july_22_unet_3d/den_3d_v{dl_version}/pred_tSN_fix3_ms_comb_mirirv3_gender/lmbd_mdiff{lambda_val_ind_mdiff}/d{dose_level}'
  ...
  hl_pat_list_fname = f'{base_folder}/final_test_pat_list_hl_gender.txt'
  hl_pat_list_gender = np.loadtxt(hl_pat_list_fname, dtype=str)

  def_pat_list_fname = f'{base_folder}/final_test_pat_list_def_gender.txt'
  def_pat_list_gender = np.loadtxt(def_pat_list_fname, dtype=str)
  ...
  pat_ind_arr_gender = {'diseased':def_pat_list_gender, 'healthy':hl_pat_list_gender}

  def_name_arr = []
  loc_arr = ['a','i']
  for diag in ['diseased', 'healthy']:
    for idx_list in range(len(pat_ind_arr[diag])):
      if pat_ind_arr_gender[diag][idx_list] != gender:
        continue
  ...
  if isIO == 1:
    ...
    fname_tSN = f'{save_folder}/tSN_seall_d{dose_level}_c30o5_IO{isIO}_Ud{Ud}_lmbdchdiff{lambda_val_ind_chdiff}_lmbdmdiff{lambda_val_ind_mdiff}_{gender}_ext45_rerun.txt'
    ...
    fname_tSN = f'{save_folder}/tSN_seall_d{dose_level}_c30o5_IO{isIO}_Ud{Ud}_lmbdchdiff{lambda_val_ind_chdiff}_lmbdmdiff{lambda_val_ind_mdiff}_{gender}_rerun.txt'
    with open(fname_tSN, 'w') as f:
      f.writelines('Initial Line\nLarge\n')
      for dd in tN:
          f.writelines(f"{dd}\n")
...
if __name__ == '__main__':
  ...
  run_cho(args.dl_version, args.Ud, args.dose_level, args.isIO, args.batch_size, args.lambda_val_ind_chdiff, args.lambda_val_ind_mdiff, args.gender)
