...
def run_cho(dl_version, Ud, dose_level, isIO, bt_size, lambda_val_ind_chdiff, lambda_val_ind_mdiff, sev_ind):
  ...
  save_folder = f'{base_folder}/learning/v1_july_22_unet_3d/den_3d_v{dl_version}/pred_tSN_fix3_ms_comb_mirirv3_def_sev/sevind_{sev_ind}/lmbd_mdiff{lambda_val_ind_mdiff}/d{dose_level}/'
  ...
  for diag in ['diseased', 'healthy']:
    for idx_list in range(len(pat_ind_arr[diag])):
      ...
      subensemble_idx = 0
      for location in ['a','i']:
        for extent in ext_arr:
          for severity in [sev_arr[sev_ind]]:
            if diag == 'diseased':
  ...
  if isIO == 1:
    ...
    fname_tSN = f'{save_folder}/tSN_seall_d{dose_level}_c30o5_IO{isIO}_Ud{Ud}_lmbdchdiff{lambda_val_ind_chdiff}_lmbdmdiff{lambda_val_ind_mdiff}_sevind_{sev_ind}_ext45_rerun.txt'
    ...
    fname_tSN = f'{save_folder}/tSN_seall_d{dose_level}_c30o5_IO{isIO}_Ud{Ud}_lmbdchdiff{lambda_val_ind_chdiff}_lmbdmdiff{lambda_val_ind_mdiff}_sevind_{sev_ind}_rerun.txt'
    with open(fname_tSN, 'w') as f:
      f.writelines('Initial Line\nLarge\n')
      for dd in tN:
          f.writelines(f"{dd}\n")
...
if __name__ == '__main__':
  ...
  parser.add_argument('--sev_ind', type=int, help="sev_ind?")
  args = parser.parse_args()
  run_cho(args.dl_version, args.Ud, args.dose_level, args.isIO, args.batch_size, args.lambda_val_ind_chdiff, args.lambda_val_ind_mdiff, args.sev_ind)
