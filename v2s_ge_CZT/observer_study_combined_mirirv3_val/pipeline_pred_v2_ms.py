...
def run_cho(dl_version, Ud, dose_level, isIO, bt_size, lambda_val_ind_chdiff, lambda_val_ind_mdiff):
  ...
  save_folder = f'{base_folder}/learning/v1_july_22_unet_3d/den_3d_v{dl_version}/pred_tSN_fix3_ms_comb_mirirv3_val/lmbd_mdiff{lambda_val_ind_mdiff}/d{dose_level}'
  #if not os.path.isdir(save_folder):
  #  os.mkdir(save_folder)
  os.makedirs(save_folder, exist_ok=True)

  inp_shape = (48, 48, 48) #change
  inp_shape_orig = (48, 48, 48) #change

  # loading patient list
  hl_pat_list_fname = f'{base_folder}/final_val_pat_list_def_split.txt'
  hl_pat_list = np.loadtxt(hl_pat_list_fname, dtype=str)

  def_pat_list_fname = f'{base_folder}/final_val_pat_list_hl_split.txt'
  def_pat_list = np.loadtxt(def_pat_list_fname, dtype=str)

  hl_pat_list_fname = f'{base_folder}/final_val_pat_list_def_src_split.txt'
  hl_pat_list_src = np.loadtxt(hl_pat_list_fname, dtype=str)

  def_pat_list_fname = f'{base_folder}/final_val_pat_list_hl_src_split.txt'
  ...
  if isIO == 1:
    ...
    fname_tSN = f'{save_folder}/tSN_seall_d{dose_level}_c30o5_IO{isIO}_Ud{Ud}_lmbdchdiff{lambda_val_ind_chdiff}_lmbdmdiff{lambda_val_ind_mdiff}_rev.txt'
