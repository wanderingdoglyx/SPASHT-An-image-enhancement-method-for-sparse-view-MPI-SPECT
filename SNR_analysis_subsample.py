...
#file_path = "/home/y.zezhang/2024_subsample_project_latest/CHO_observer_study_nn2/patient_selection.txt"
...
#diseased_patient_ids = np.array(diseased_patient_ids, dtype=str)
#healthy_patients=np.array(healthy_patients, dtype=str)
...
base_folder = f'/data01/user-storage/y.zezhang/2024_subsample_project/mod_SA_images'##########################
...
save_folder = f'{save_base_folder}/SNR/d{subsample_level}/'###########
...
pat_ind_arr = {'diseased':diseased_patient_ids , 'healthy':healthy_patients}
...
for location in location_setting:
  for extent in extent_setting:
    for severity in severity_setting:
          for diag in ['diseased', 'healthy']:
            for index, di_item in enumerate(diseased_patients):
              ...
              if diag == 'diseased':
                ...
                cur_path=os.path.join(base_folder,str(subsample_level))
                cur_path=os.path.join(cur_path,diag)
                cur_path=os.path.join(cur_path,patient)
                cur_path=os.path.join(cur_path,CT_method)
                cur_path=os.path.join(cur_path,def_name)
          ...
          np.save(save_folder +'delta_f_bar'+'_d'+str(subsample_level)+'_location'+str(location)+"_extent"+str(extent)+'_severity'+str(severity),delta_f_bar)
          np.save(save_folder +"delta_v_bar"+'_d'+str(subsample_level)+'_location'+str(location)+"_extent"+str(extent)+'_severity'+str(severity),delta_v_bar)
          np.save(save_folder + "K"+'_d'+str(subsample_level)+'_location'+str(location)+"_extent"+str(extent)+'_severity'+str(severity),K)
          np.save(save_folder +'IS'+'_d'+str(subsample_level)+'_location'+str(location)+"_extent"+str(extent)+'_severity'+str(severity),np.mean(IS,axis=1))
          np.save(save_folder +"IN"+'_d'+str(subsample_level)+'_location'+str(location)+"_extent"+str(extent)+'_severity'+str(severity),np.mean(IN,axis=1))
          ...
          mdic={'delta_f_bar'+'_d'+str(subsample_level): delta_f_bar,
                "delta_v_bar"+'_d'+str(subsample_level): delta_v_bar,
                "K"+'_d'+str(subsample_level): K,
                "IS"+'_d'+str(subsample_level): np.mean(IS,axis=1),
                "IN"+'_d'+str(subsample_level):np.mean(IN,axis=1) }
                

          savemat(save_folder +'SNR_data'+'_d'+str(subsample_level)+'_location'+str(location)+"_extent"+str(extent)+'_severity'+str(severity)+'.mat', mdic)
