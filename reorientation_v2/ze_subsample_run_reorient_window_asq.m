...
for AC_method_id = 1:length(AC_arr)
    ...
    patient_list_path='/data01/user-storage/y.zezhang/2024_subsample_project/mod_subsample_projection/30/hl_asq';
    patient_list = split(ls(patient_list_path));
    patient_list = patient_list(1:end-1);

    ze_subsample_s1_reorient_func_asq(patient_list,def_name, AC_method,'healthy',num2str(sample_slices))

end

%% disease patient to healthy patient
...
for AC_method_id = 1:length(AC_arr)
    ...
    file_path=['/data01/user-storage/y.zezhang/2024_subsample_project/mod_reconstruction/',num2str(sample_slices),'/',AC_method];
    patient_list_path='/data01/user-storage/y.zezhang/2024_subsample_project/mod_subsample_projection/30/def_asq';
    patient_list = split(ls(patient_list_path));
    patient_list = patient_list(1:end-1);
    ...
    for def_ext = ext_arr 
        for severity_idx = 1:length(sev_arr)
            ...
            for AC_method_id = 1:length(AC_arr)
                ...
                ze_subsample_s1_reorient_func_asq(patient_list,def_name, AC_method, 'diseased',num2str(sample_slices))
