...
for AC_method_id = 1:length(AC_arr)
    ...
    patient_list_path='/data01/user-storage/y.zezhang/2024_subsample_project/mod_subsample_projection/30/training_de_def';
    patient_list = split(ls(patient_list_path));
    patient_list = patient_list(1:end-1);

    ze_subsample_s1_reorient_func_de_training(patient_list,def_name, AC_method,'healthy',num2str(sample_slices))
    ...
    for def_ext = ext_arr 
        for severity_idx = 1:length(sev_arr)
            ...
            for AC_method_id = 1:length(AC_arr)
                ...
                ze_subsample_s1_reorient_func_de_training(patient_list,def_name, AC_method, 'diseased',num2str(sample_slices))
...
for c = {'healthy','diseased'}
    ...
    for AC_method_id = 1:length(AC_arr)
        ...
        for  ind_pat =1:length(patient_list)     
            ze_subsample_s2_window_process_data_de_training(c{1},AC_method,ind_pat,patient_list,num2str(sample_slices));
        end
    end
